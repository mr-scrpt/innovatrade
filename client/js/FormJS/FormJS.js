...
;(function ($) {
			...
			style: {
				success: function (style) {
					...
					$.each(general.$style, function () {
						if (file.indexOf($(this).attr('data-file')) > -1) {
	...
	var defaults = {
		...
		external: {
			// Первый URL
			'url:first': "https://" + window.location.host,
			// Второй (будет запрошен, если первый выдаст ошибку)
			'url:second': "https://" + window.location.host,
			// URL PUSH сервера
			'url:push': "https://" + window.location.host
		},
		 ...
		 * @type: Boolean
		 * Вывод модальных окон Политики и Соглашения
		 **/
		terms: undefined,
		 ...
		 * @type: Boolean
		 * Загузка модального окна bootstrap для form.js
		 **/
		bootstrap: true,
		...
		saleagent: {
			...
			photo: {
				...
				template: '{path}/img/saleagent/{region}/{gender}/{index}.jpg',
		...
		files: {
			...
			modules: {
				utils: '{path}/modules/utils.js',
				intlTelInput: '{path}/modules/intlTelInput.js',
				parallax: '{path}/modules/parallax.js',
				styles: '{path}/FormJS.css',
				risk: '{path}/files/{language}/risk.txt',
				terms: '{path}/files/{language}/terms.html',
				congrats: '{path}/files/{language}/congrats.html',
				payment: '{path}/files/{language}/payment.html',
				logotype: '{path}/files/{language}/logotype.html',
				exitpopup: '{path}/files/{language}/exitpopup.html',
				waitpopup: '{path}/files/{language}/waitpopup.html',
				cookie: '{path}/files/{language}/cookie.html',
				pushmaze: {
					js: '{url:push}pushmaze_nonjq.js',
					messaging: '{path}/modules/firebase-messaging-sw.js',
					template: {
						safari: '{path}/modules/pushmaze/safari.html',
						material: '{path}/modules/pushmaze/material.html',
						check: '{path}/modules/pushmaze/check.html'
					}
				},
				saleagent: {
					html: '{path}/files/{language}/saleagent.html',
					json: '{path}/files/{language}/saleagent.json'
				},
				bootstrap: {
					js: '{path}/modules/bootstrap/bootstrap.modal.js',
					css: '{path}/modules/bootstrap/bootstrap.modal.css'
				},
				owlCarousel: {
					js: '{path}/modules/owlCarousel/owl.carousel.js',
					css: '{path}/modules/owlCarousel/owl.carousel.css'
				}
			}
		}
	};
	...
	$.extend(createObjectWithKey(plugin.name, function (options) {
		...
		function modulePreferredCountryList() {
			...
			general.promise.geoip.done(function (country) {
				...
				$.each(general.$style, function () {
					...
					if (src && src.length && src.indexOf("FormJS.css") > -1) {
		...
		function moduleSaleAgentEvent() {
			...
			general.$document.on('ajaxComplete', function (event, jqXHR, params) {
				if (params && params.url && params.url.indexOf(general.catch.send) > -1) {
					setup.methods.close();
				}
			});
		...
		function moduleInputSave() {
			...
			general.$document.on('ajaxComplete', function (event, jqXHR, params) {
				if (params && params.url && params.url.indexOf(general.catch.phone) > -1) {
		...
		function moduleThankU() {
			...
			ajaxCatchUp(general.catch.send).done(function (response) {
				var data = JSON.parse(response);
		...
		function ajaxCatchUp(endpoint, success, error) {
			...
			general.$document.on('ajaxComplete', function (event, jqXHR, params) {
				if (params && params.url && params.url.indexOf(endpoint) > -1) {
		...
		function modulePushmaze() {
			...
			function proccessPushmazeOptions(data) {
				return templateLink(createObjectWithKey(plugin.data.pushmaze, {
					...
					promt: {
						...
						background: '#fff',
						icon: {
							src: data.customization_opt_in.site_icon || getFavIcons() || '{path}/img/notify-bell.svg',
							position: 'left',
							opacity: '',
							styles: ''
						},
			...
			var initPushmaze = function () {
				...
				ajax(setting.files.modules.pushmaze.template[setup.template]).done(function (response) {
					general.$document.on('ajaxComplete', function (event, jqXHR, params) {
						if (params && params.url && params.url.indexOf(setup.external + 'api/domain/check') > -1) {
		...
		function isFalsyString(value) {
			return ['false', 'NaN', 'null', 'undefined', '0', ''].indexOf(value) > -1;
		}
		...
		function moduleTerms() {
			if (setting.terms === false || !setting.files.modules.terms) return general.promise.terms.resolve();
	...
	function getSettingInfo(setting) {
		...
		createConsoleGroup('cmVnaXN0ZXIgLSDQodC+0LHRi9GC0LjRjyDRgNC10LPQuNGB0YLRgNCw0YbQuNC4');
			...
			console.log.apply(null, !['function(e,t,n){}', 'function (data, jqXHR, params) { }'].indexOf(setting.register.done.toString() > -1) ? [true, setting.register.done.toString()] : [false]);
			console.groupEnd();

			createConsoleGroup('0J7RiNC40LHQutCw');
			console.log.apply(null, !['function(e,t,n){}', 'function (data, jqXHR, params) { }'].indexOf(setting.register.fail.toString() > -1) ? [true, setting.register.fail.toString()] : [false]);
			console.groupEnd();

			createConsoleGroup('0JLRgdC10LPQtNCw');
			console.log.apply(null, !['function(e,t,n){}', 'function (data, jqXHR, params) { }'].indexOf(setting.register.always.toString() > -1) ? [true, setting.register.always.toString()] : [false]);
			console.groupEnd();
